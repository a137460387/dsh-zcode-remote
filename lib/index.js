...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Send a task prompt to a ZCode client on another machine through its mobile remote-control relay, like the phone app does. '
      ...
      + 'A `new_task` dispatch starts on `model` at reasoning level `thought` (defaults: GLM-5.3-Flash at max); the result\'s `config` field '
      + 'reports what the desktop actually applied, since invalid values are silently replaced by its defaults.',
    parameters: {
