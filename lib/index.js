...
export function apply(ctx, config) {
  ...
  /**
   * Run one client-scoped operation, releasing only the addressed client's
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Fetch the reply of a task started with zcode_remote_dispatch({ async: true }), and release its subscription. '
      + 'Give the same `device` or `url` used to dispatch, plus the `session_id` that dispatch returned. '
      + 'Waits until the client has been silent for a moment, or until `wait_seconds` runs out. '
      + 'If the task is unknown (already collected, or this plugin restarted), falls back to the task\'s latest assistant text.',
    parameters: {
      ...
      return onDevice(args, async (session, target) => {
        ...
        if (!entry) {
          throw new Error(`no in-flight dispatch for task ${args.session_id} on ${label ?? target.url}. `
            + 'It was either already collected, dispatched without `async: true`, or started before this plugin restarted.')
        }
        inflight.delete(dispatchKey(target.url, args.session_id))
