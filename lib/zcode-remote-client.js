...
export class ZcodeRemoteSession {
  ...
  /**
   * Wait for one dispatch's reply. Completion is 12 s of frame silence after at
   * least one new row, capped by `waitMs`.
  ...
  async collectDispatch(handle, { waitMs = 180000, signal } = {}) {
    ...
    while (Date.now() - start < waitMs) {
      ...
      if (Date.now() - handle.lastChange > 12000 && this.replyRows(handle).length) break
    }
    if (this.dispatchHandlers.get(handle.key) === handle) this.dispatchHandlers.delete(handle.key)
    // Release the subscription: a client reused for many dispatches would
    // otherwise accumulate them, and the desktop keeps streaming to each.
