...
export class RpcAssembler {
  ...
  accept(frame) {
    if (frame.seq !== this.expectedSeq || frame.messageSeq !== this.expectedMessageSeq) {
      // Resync on evidence of loss rather than corruption: a seq AHEAD of
      // expected (the sender kept going past a frame we never saw), or the same
      // mismatch twice in a row (the sender is re-anchoring). One gap is then
      // tolerated — the middle of a multi-fragment message is a single gap,
      // not two. Without this the channel dies on the first lost frame.
      const gap = `${frame.seq}:${frame.messageSeq}`
      const shouldResync = frame.seq > this.expectedSeq || this.lastGap === gap
      if (shouldResync) {
        const reason = `resync from seq=${this.expectedSeq} messageSeq=${this.expectedMessageSeq} to seq=${frame.seq} messageSeq=${frame.messageSeq}`
        this.expectedSeq = frame.seq
        this.expectedMessageSeq = frame.messageSeq
        this.active = null
        this.lastGap = null
...
export class ZcodeRemoteClient {
  ...
  async connect() {
    ...
    this.ws.addEventListener('close', ev => this.onRelayClose(ev.code, ev.reason || ''))
  ...
  /**
   * Fail every waiter and in-flight RPC when the socket dies. A dead socket is
   * not "the desktop is slow": callers must reject now, not at their own
   * 30-60 s timeout, so they can tell the connection died from a slow reply.
   * @param {number} code - the WebSocket close code.
   * @param {string} reason - the WebSocket close reason.
   */
  onRelayClose(code, reason) {
    this.log(`relay closed code=${code} reason=${reason}`)
    this.stopHeartbeat()
    this.state = 'closed'
    for (const waiter of this.relayWaiters.splice(0)) {
      clearTimeout(waiter.timer)
      waiter.reject(new Error(`relay closed while waiting (${code})`))
    }
    const rpcError = new Error(`relay closed (${code})`)
    for (const handler of this.channelHandlers.values()) {
      handler.reject(rpcError)
    }
    this.channelHandlers.clear()
    this.initialized = false
    for (const waiter of this.initWaiters.splice(0)) {
      clearTimeout(waiter.timer)
  ...
  onRpcFramePayload(payload) {
    ...
    if (result.kind === 'message') {
      ...
      // Sequence counters moved without us: anything in flight is already lost
      // (its reply landed in the missing range or its id was superseded), so
      // reject it fast instead of waiting out 30-60 s timeouts, and let the
      // next RPC re-run the handshake on the recovered channel.
      this.log(`rpc assembler ${result.reason}`)
      const error = new Error('rpc channel resynchronized after frame loss')
      for (const handler of this.channelHandlers.values()) handler.reject(error)
      this.channelHandlers.clear()
      this.initialized = false
      for (const waiter of this.initWaiters.splice(0)) { clearTimeout(waiter.timer); waiter.reject(error) }
  ...
  onChannelBytes(bytes) {
    ...
    switch (type) {
      case 200:
        ...
        for (const w of this.initWaiters.splice(0)) { clearTimeout(w.timer); w.resolve() }
        break
      case 201:
  ...
  waitInitialized(timeoutMs = 30000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      waiter.timer = setTimeout(() => {
        const i = this.initWaiters.indexOf(waiter)
        if (i >= 0) this.initWaiters.splice(i, 1)
        reject(new Error('rpc Initialize timeout'))
      }, timeoutMs)
   ...
   * @param {{ provider?: string, model?: string, thought?: string }} [config] -
   *   the model selection the new task starts with, sent as the legacy `config`
   *   envelope. The desktop silently replaces invalid values with its own
   *   defaults, so the subscription snapshot's `config` — not this payload — is
   *   the authority on what applied. Do NOT add `firstInput.modelSelection`
   *   here: the desktop's createSession path accepts it in its schema but
   *   stalls the task without ever starting it (verified live, 3.12.3).
...
/**
 * Build the createSession `config` envelope from a model selection, dropping
 * unspecified fields, or undefined when nothing is specified — the payload then
 * leaves the model choice to the desktop's workspace defaults.
 * @param {{ provider?: string, model?: string, thought?: string }} [selection]
 * @returns the config object, or undefined when the selection is empty.
 */
export function sessionConfig(selection) {
  const config = {}
  if (selection?.provider !== undefined) config.provider = selection.provider
  if (selection?.model !== undefined) config.model = selection.model
  if (selection?.thought !== undefined) config.thought = selection.thought
...
/**
 * An error raised BY the call — a bad selector, an unexpected reply — rather
 * than by the connection. Mirrors `UsageError` in the plugin layer without
 * importing it (this module must stay dependency-free); `onDevice` matches by
 * class name.
 */
...
export class ZcodeRemoteSession {
  ...
  async ensureClient() {
    ...
    this.connecting = (async () => {
      try {
        this.client?.close()
        // Bridges, frame routers and the agent handshake belong to the old
        // socket: reusing them leaves the new client un-initialized (every RPC
        // waits for an ack that never comes) and routes frames to subscriptions
        // the relay no longer knows, so later calls stall.
        this.bridges.clear()
  ...
  selectWorkspaceKey(wsList, wanted) {
    ...
    if (wanted) {
      ...
      if (!entry) {
        ...
        throw new UsageError(`workspace "${wanted}" is not open on this client. Open workspaces: ${open}`)
      }
   ...
   * @param {{ text: string, sessionId?: string, workspace?: string, newTask?: boolean,
   *   model?: string, thought?: string, provider?: string }} request - the prompt,
   *   target, and — for a new task — the model selection it starts with.
   * @returns the dispatch handle to pass to {@link collectDispatch}.
   */
  async startDispatch({ text, sessionId, workspace, newTask = false, model, thought, provider }) {
    const { client, bridge, workspaceKey } = await this.ensureReady(workspace)
    ...
    if (newTask) {
      ...
      const created = await client.sendConversationCommand(
        bridge.workspacePath,
        client.makeNewSessionCommand(workspaceKey, text, sessionConfig({ provider, model, thought })),
      )
      const target = created?.result?.sessionId ?? created?.sessionId
      if (!target) {
        throw new UsageError(`createSession returned no task id: ${JSON.stringify(created ?? null).slice(0, 300)}`)
      }
      ...
      const handle = {
        taskId: target,
        subscriptionId,
        // Without a desktop-issued id the key must be unique per handle: two
        // dispatches into one task sharing `session:<target>` would overwrite
        // each other's routing. Routing then falls to the single-active-
        // dispatch rule in ensureFrameRouter.
        key: subscriptionId ?? `session:${target}:${randomUUID()}`,
      ...
      // The session was created BY this dispatch: every row it will ever contain
      // is this prompt's output, so nothing is pre-existing — `before` stays
      // empty and no snapshot wait is needed. A fixed wait here would race a
      // fast first reply into the excluded set.
      return handle
    }
  ...
  /**
   * Re-attach to a task WITHOUT sending anything: subscribe to its conversation
   * and return a handle whose rows fill from the subscription snapshot and
   * deltas. `before` stays empty — every row the conversation holds is this
   * task's own output — so collect reports the transcript as a whole. This is
   * the recovery path when this plugin restarted and the in-memory dispatch
   * registry no longer knows the task; the desktop kept running (or finished)
   * it either way.
   * @param {string} sessionId - the task id (sess_*).
   * @param {string} [workspace] - optional workspace selector.
   * @returns the dispatch handle to pass to {@link collectDispatch}.
   */
  async reattachDispatch(sessionId, workspace) {
    if (!sessionId) throw new Error('reattach needs a task id (sess_*)')
    const { client, bridge, workspaceKey } = await this.ensureReady(workspace)
    this.ensureFrameRouter(client, bridge)
    const sub = await client.subscribeConversation(bridge.workspacePath, sessionId)
    const subscriptionId = sub?.ack?.subscriptionId ?? null
    const handle = {
      taskId: sessionId,
      subscriptionId,
      key: subscriptionId ?? `session:${sessionId}:${randomUUID()}`,
      workspacePath: bridge.workspacePath,
      workspaceKey,
      rows: new Map(),
      before: new Set(),
  ...
  absorbInto(handle, payload) {
    ...
    if (payload.kind === 'snapshot' && Array.isArray(payload.snapshot?.rows?.window)) {
      // The snapshot names the config the desktop actually applied — the
      // authority on a model selection, whose invalid values it silently
      // replaces with its own defaults.
      const applied = payload.config ?? payload.snapshot.config
      if (applied && typeof applied === 'object') handle.effectiveConfig = applied
  ...
  async resubscribe(handle) {
    ...
    // A reconnect clears frameRouters: re-register the router BEFORE the new
    // subscription exists, or its frames arrive with nothing routing them and
    // this collect's safety net goes permanently blind. Idempotent per bridge.
    this.ensureFrameRouter(client, bridge)
    const previousId = handle.subscriptionId
    ...
    if (nextId && nextId !== previousId) {
      ...
      handle.key = nextId
      this.dispatchHandlers.set(handle.key, handle)
      // Release the superseded subscription, or every 30 s safety-net cycle
      // leaves one more zombie on the desktop. Failure is logged, not fatal:
      // frames the old subscription receives from here on are dropped, and the
      // fresh snapshot replays the state anyway.
      if (previousId) {
        try {
          await client.unsubscribeConversation(bridge.workspacePath, previousId)
        } catch (error) {
          this.log(`unsubscribe failed for ${previousId}: ${error?.message ?? error}`)
        }
      }
    }
  }
  ...
  async collectDispatch(handle, { waitMs = 180000, signal } = {}) {
    ...
    const result = {
      ...
      transcript,
      complete: settled,
      ...(handle.effectiveConfig ? { config: handle.effectiveConfig } : {}),
    }
